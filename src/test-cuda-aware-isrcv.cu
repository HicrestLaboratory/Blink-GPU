    ...
    for(int i=0; i<=27; i++){
        ...
        */
        for(int i=1; i<=loop_count; i++){
            if(rank == 0){
		    MPI_Isend(d_A, N, MPI_DOUBLE, 1, tag1, MPI_COMM_WORLD, &req[0]);
		    MPI_Irecv(d_A, N, MPI_DOUBLE, 1, tag2, MPI_COMM_WORLD, &req[1]);
            }
            else if(rank == 1){
		    MPI_Irecv(d_A, N, MPI_DOUBLE, 0, tag1, MPI_COMM_WORLD, &req[0]);
		    MPI_Isend(d_A, N, MPI_DOUBLE, 0, tag2, MPI_COMM_WORLD, &req[1]);
            }

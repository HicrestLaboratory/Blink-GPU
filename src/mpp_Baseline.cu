...
//     cudaErrorCheck( cudaSetDevice(rank % num_devices) );
    ...
    if (ppComm != MPI_COMM_NULL) {
            ...
            */
            ...
            for(int i=1-(WARM_UP); i<=loop_count; i++){
                ...
                if(rank < my_peer){
                    ...
                    MPI_Isend(A, N, MPI_dtype, my_peer, tag1, MPI_COMM_WORLD, &(request[allppComm_rank]));
                    MPI_Recv(B, N, MPI_dtype, my_peer, tag2, MPI_COMM_WORLD, &stat);
                    cudaErrorCheck( cudaMemcpy(d_B, B, N*sizeof(dtype), cudaMemcpyHostToDevice) );
                } else {
                    MPI_Recv(B, N, MPI_dtype, my_peer, tag1, MPI_COMM_WORLD, &stat);
                    cudaErrorCheck( cudaMemcpy(d_B, B, N*sizeof(dtype), cudaMemcpyHostToDevice) );
                    cudaErrorCheck( cudaMemcpy(A, d_A, N*sizeof(dtype), cudaMemcpyDeviceToHost) );
                    MPI_Isend(A, N, MPI_dtype, my_peer, tag2, MPI_COMM_WORLD, &(request[allppComm_rank]));
                }
        ...
        MPI_Allreduce(inner_elapsed_time, elapsed_time, buff_cycle*loop_count, MPI_DOUBLE, MPI_MAX, allppComm);
        for(int j=fix_buff_size; j<max_j; j++) {
            long int N = 1 << j;

...
//     cudaErrorCheck( cudaSetDevice(rank % num_devices) );
    ...
    for (int i=0; i<NSTREAM; i++) {
        cudaErrorCheck( cudaStreamCreate(&stream[i]) );
    }
    ...
    if (rank == 0 || rank == rank2) {
        for(int j=0; j<BUFF_CYCLE; j++){
            ...
            dtype *A0, *A1, *B0, *B1;
            cudaErrorCheck( cudaMallocHost(&A0, N*sizeof(dtype)) );
            cudaErrorCheck( cudaMallocHost(&A1, N*sizeof(dtype)) );
            cudaErrorCheck( cudaMallocHost(&B0, N*sizeof(dtype)) );
            cudaErrorCheck( cudaMallocHost(&B1, N*sizeof(dtype)) );
            ...
            */
            for(int i=1; i<=loop_count; i++){
                ...
                if(rank == 0){
                    cudaErrorCheck( cudaMemcpyAsync(A0, d_A0, N*sizeof(dtype), cudaMemcpyDeviceToHost, stream[0]) );
                    cudaErrorCheck( cudaStreamSynchronize(stream[0]) );
                    cudaErrorCheck( cudaMemcpyAsync(A1, d_A1, N*sizeof(dtype), cudaMemcpyDeviceToHost, stream[1]) );
                    MPI_Isend(A0, N, MPI_dtype, rank2, tag1, MPI_COMM_WORLD, &req[0]);
                    cudaErrorCheck( cudaStreamSynchronize(stream[1]) );
                    MPI_Isend(A1, N, MPI_dtype, rank2, tag2, MPI_COMM_WORLD, &req[1]);

                    /* Implicit syncronization, rank2 wait for receving before sending */

                    MPI_Irecv(B0, N, MPI_dtype, rank2, tag3, MPI_COMM_WORLD, &req[0]);
                    MPI_Wait(&req[0], &status[0]);
                    MPI_Irecv(B1, N, MPI_dtype, rank2, tag4, MPI_COMM_WORLD, &req[1]);
                    cudaErrorCheck( cudaMemcpyAsync(d_B0, B0, N*sizeof(dtype), cudaMemcpyHostToDevice, stream[0]) );
                    MPI_Wait(&req[1], &status[1]);
                    cudaErrorCheck( cudaMemcpyAsync(d_B1, B1, N*sizeof(dtype), cudaMemcpyHostToDevice, stream[1]) );
                    cudaErrorCheck( cudaDeviceSynchronize() );
                }
                else if(rank == rank2){
                    MPI_Irecv(B0, N, MPI_dtype, 0, tag1, MPI_COMM_WORLD, &req[0]);
                    MPI_Wait(&req[0], &status[0]);
                    MPI_Irecv(B1, N, MPI_dtype, 0, tag2, MPI_COMM_WORLD, &req[1]);
                    cudaErrorCheck( cudaMemcpyAsync(d_B0, B0, N*sizeof(dtype), cudaMemcpyHostToDevice, stream[0]) );
                    MPI_Wait(&req[1], &status[1]);
                    cudaErrorCheck( cudaMemcpyAsync(d_B1, B1, N*sizeof(dtype), cudaMemcpyHostToDevice, stream[1]) );
                    ...
                    cudaErrorCheck( cudaStreamSynchronize(stream[0]) );
                    cudaErrorCheck( cudaMemcpyAsync(A1, d_A1, N*sizeof(dtype), cudaMemcpyDeviceToHost, stream[1]) );
                    MPI_Isend(A0, N, MPI_dtype, 0, tag3, MPI_COMM_WORLD, &req[0]);
                    cudaErrorCheck( cudaStreamSynchronize(stream[1]) );
                    MPI_Isend(A1, N, MPI_dtype, 0, tag4, MPI_COMM_WORLD, &req[1]);
                }
            ...
            if(rank == 0){
                ...
                MPI_Recv(&recv_cpu_check0, 1, MPI_cktype, rank2, tag3, MPI_COMM_WORLD, &stat);
                MPI_Recv(&recv_cpu_check1, 1, MPI_cktype, rank2, tag4, MPI_COMM_WORLD, &stat);
            } else if(rank == rank2){
                MPI_Recv(&recv_cpu_check0, 1, MPI_cktype, 0, tag1, MPI_COMM_WORLD, &stat);
                MPI_Recv(&recv_cpu_check1, 1, MPI_cktype, 0, tag2, MPI_COMM_WORLD, &stat);
            ...
            fflush(stdout);
            cudaErrorCheck( cudaFreeHost(A0) );
            cudaErrorCheck( cudaFreeHost(A1) );
            cudaErrorCheck( cudaFreeHost(B0) );
            cudaErrorCheck( cudaFreeHost(B1) );
    ...
    for (int i=0; i<NSTREAM; i++) {
        cudaErrorCheck( cudaStreamDestroy(stream[i]) );
    }

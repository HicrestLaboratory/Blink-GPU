...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0 || rank == rank2) {
        for(int j=fix_buff_size; j<max_j; j++){
            ...
            MPI_Win d_A_win, d_B_win;
            MPI_Win_create(d_A, N*sizeof(dtype), sizeof(dtype), MPI_INFO_NULL, ppComm, &d_A_win);
            MPI_Win_create(d_B, N*sizeof(dtype), sizeof(dtype), MPI_INFO_NULL, ppComm, &d_B_win);
            ...
            */
            ...
            for(int i=1-(WARM_UP); i<=loop_count; i++){
                ...
                MPI_Win_fence(0, d_B_win);
                if(rank == 0) MPI_Put(d_A, N, MPI_dtype, 1, 0, N, MPI_dtype, d_B_win);
                MPI_Win_fence(0, d_B_win);

                MPI_Win_fence(0, d_A_win);
                if(rank == 0) MPI_Get(d_B, N, MPI_dtype, 1, 0, N, MPI_dtype, d_A_win);
                MPI_Win_fence(0, d_A_win);
            ...
            fflush(stdout);
            MPI_Win_free(&d_A_win);
            MPI_Win_free(&d_B_win);

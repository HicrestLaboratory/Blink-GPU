...
//     }
    ...
    if(rank == 0){
        ...
        dtype *A;
#ifdef PINNED
        cudaHostAlloc(&A, elems_in_buffer*sizeof(dtype), cudaHostAllocDefault);
#else
        ...
        */
        ...
        for(int i=1-(WARM_UP); i<=loop_count; i++){
            ...
            ncclGroupStart();
            // Assume the root of the incast is rank 0
            if(rank == 0){
                for (int r=1; r<size; r++){
                    ncclRecv(d_A + (r*N)*sizeof(dtype), N, ncclDtype, r, NCCL_COMM_WORLD, NULL);
                }
            }else{

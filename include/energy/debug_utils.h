...
#define STR_COLL_INIT {                                           \
  str_coll.mycollector = (char*)malloc(sizeof(char)*STR_COLL_BLK);  \
  str_coll.buff = (char*)malloc(sizeof(char)*STR_COLL_BLK);       \
  str_coll.mycollector[0]='\0';                                     \
...
#define STR_COLL_APPEND(X) {                                                              \
  ...
  if (str_coll.len + n >= str_coll.size) {                                                \
    str_coll.mycollector = (char*)realloc(str_coll.mycollector, str_coll.size + STR_COLL_BLK);\
    str_coll.size += STR_COLL_BLK;                                                        \
  }                                                                                       \
  memcpy(&str_coll.mycollector[str_coll.len], str_coll.buff, sizeof(char)*n);               \
  str_coll.len += n;                                                                      \
  str_coll.mycollector[str_coll.len]='\0';                                                  \
  memset(str_coll.buff, 0, sizeof(char) * STR_COLL_BLK);                                  \
}

#define STR_COLL_LEN  str_coll.len
#define STR_COLL_BUFF str_coll.buff
#define STR_COLL_SIZE str_coll.size
#define STR_COLL_GIVE str_coll.mycollector

#define STR_COLL_FREE {       \
    free(str_coll.mycollector); \
    free(str_coll.buff);      \
    str_coll.size = 0;        \
    str_coll.len = 0;         \
}

#define COUNTER_DEF(n) int macro_counter_##n=0;
#define COUNTER_INCREASE(n) macro_counter_##n ++;
#define COUNTER_READ(n) macro_counter_##n
